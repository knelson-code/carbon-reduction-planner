...
export default function ClientResiliencePage() {
  const { data: session, status } = useSession()
  const router = useRouter()

  useEffect(() => {
    if (status === "unauthenticated") {
      router.push("/login?callbackUrl=/climate-risk-management/analyze-risks/client-resilience")
    }
  }, [status, router])
    ...
    <div className="flex min-h-[calc(100vh-92px)]">
      ...
      <div className="flex-1 bg-gray-50 p-8">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-3xl font-bold mb-6" style={{ color: '#0B1F32' }}>Decline in Resilience of Our Clients</h1>
